@echo off
rem Builds tests\bp3clocks.exe, the harness that sends every entry of the
rem Buzzpirat / BusPirateV3 SPI clock menu to real hardware and checks that each
rem one still reads the chip. It covers the menu to firmware index mapping in
rem buzzpirathw.pas, which is not the same table on a real Bus Pirate v3 as it
rem is on the v5 and newer legacy emulation.
rem
rem By Dreg
...
rem
rem Run from the software\ directory:  tests\build_bp3clocks.cmd
...
"%FPC%" -Mobjfpc -Sh -O2 ^
  ...
  -Fu"%MPHEX%" ^
  -Filib\i386-win32 -FU"%OUT%" -FE"%OUT%" -o"%OUT%\bp3clocks.exe" tests\bp3clocks.lpr

if errorlevel 1 (
  echo BUILD FAILED
  exit /b 1
)
echo Built %OUT%\bp3clocks.exe
endlocal
